...
find_secondary_site <- function(acceptor_sites, feature, gene, gene_strand,
                               smoothed=FALSE) {
    # If zero or one sites are found, then there is no secondary site detected
    if (length(acceptor_sites) <= 1) {
        return(list(location=NA, num_reads=NA))
    }

    # find second highest peak
    secondary_site_index <- find_peak(gene, acceptor_sites, gene_strand,
                                     feature, smoothed=smoothed, secondary=TRUE)
    if (smoothed) {
...
get_utr_sequences <- function(genes, fasta, utr_lengths, utr5=TRUE) {
    ...
    widths <- data.frame(id=genes$ID, width=NA)
