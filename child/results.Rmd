...
create_utr_comparison_df <- function(stage1, stage2) {
    ...
    comparison_df <- tbl_df(data.frame(
        ...
        min_primary_site_num_reads=min(df1$num_reads_primary, df2$num_reads_primary),
        min_ptos_ratio=min(df1$num_reads_primary / df1$num_reads_secondary, 
                           df2$num_reads_primary / df2$num_reads_secondary),
        count_min=min(df1$num_reads_primary, df2$num_reads_primary),
    ...
    comparison_df %>%
        ...
        mutate(
            log_min_ratio=log2(min(stage1_stage2_ratio_stage1_samples, stage2_stage1_ratio_stage2_samples)),
            length_diff=abs(stage1_len - stage2_len), 
            log_min_reads=log2(min_primary_site_num_reads),
    ...
    for (s2 in stages) {
        ...
        plt <- ggplot(utr_comparison_df, 
            aes(stage1_len, stage2_len, color=log_min_ptos,
                size=min_primary_site_num_reads)) + 
            geom_point() +
            xlab(s1) + ylab(s2) +
            ...
            geom_text(data=subset(utr_comparison_df, log_min_ptos >= 2 & min_primary_site_num_reads >= 50 & len_diff > 10),
        ...
        if (nrow(filtered_df) > 0) {
            ...
            plt <- ggplot(filtered_df,
                ...
                scale_x_continuous(expand=c(0.01, 0.01)) +
                scale_y_continuous(expand=c(0.01, 0.01)) +
                geom_text(data=subset(utr_comparison_df, log_min_ptos >= 2 & log_min_reads >= 3 & len_diff > 10),
